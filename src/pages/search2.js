...
export default function Search2Page({ data }) {
  ...
  return (
    <Layout dark>
      ...
      <br />
      <br />
      <Link to="/search" css={css`display: inline-block; font-style: normal;`}>
      ...
      {results.map(({ node }) => (
        <div key={node.id} css={css`margin-bottom: ${rhythm(1)};`}>
          <Link
            to={node.frontmatter.backroad || node.fields.slug}
            css={css`
            ...
            `}
          >
            <h2 css={css`margin-bottom: ${rhythm(1 / 4)}; color: inherit;`}>
